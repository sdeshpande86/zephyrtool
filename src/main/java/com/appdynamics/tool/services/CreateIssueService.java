...
public class CreateIssueService {
	...
	public Response createIssue(@QueryParam("usecase") String usecase, @QueryParam("parentissue") String parentIssue) throws InterruptedException, URISyntaxException {
		...
		return Response.temporaryRedirect(new URI(App.JIRA_BASE_URL + "/secure/CreateIssueDetails!init.jspa?pid="
				+ App.PROJECT_ID + "&issuetype=" + App.ID_FOR_ISSUE_TYPE_TEST + "&" + App.TESTTYPE_FIELD_ID + "="
				+ App.ID_FOR_TEST_TYPE_FUNCTIONALITY + "&" + App.AUTOMATED_FIELD_ID + "=" + App.ID_FOR_AUTOMATED_NO
				+ "&issuelinks-linktype=sets%20requirements%20for&" + App.USECASE_FIELD_ID + "="
				+ App.usecaseValueToIdMap.get(usecase))).build();
 	}
}